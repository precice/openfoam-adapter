...
void preciceAdapter::Adapter::setupCheckpointing()
{
    ...
    // The velocity field U is checkpointed via its internal values (and
    // old-times) instead of a GeometricField clone, see UField_ comment.
    if (mesh_.foundObject<volVectorField>("U"))
    {
        UField_ = &const_cast<volVectorField&>(mesh_.lookupObject<volVectorField>("U"));
        UInternalCopies_ = UField_->internalField();
        UnOldTimes_ = UField_->nOldTimes();
        if (UnOldTimes_ >= 1)
        {
            UOldInternalCopies_ = UField_->oldTime().internalField();
        }
        DEBUG(adapterInfo("Checkpoint U via internal-value buffer (nOldTimes = " + std::to_string(UnOldTimes_) + ")."));
    }
...
#define doLocalCode(GeomField)                                                                      \
    ...
    {                                                                                 \
        /* Skip FV mesh geometry fields, which are regenerated by fvMesh:move() */                 \
        /* and U, handled by its own internal-value checkpoint above */                            \
        if (fvMesh::geometryFields.found(obj) || obj == "U"){                                       \
            continue;                                                                               \
        }                                                                                           \
        addCheckpointField(&(const_cast<GeomField&>(mesh_.lookupObject<GeomField>(obj)))); \
...
void preciceAdapter::Adapter::readCheckpoint()
{
    ...
    // Reload the velocity field U from its internal-value checkpoint
    if (UField_ != nullptr)
    {
        UField_->primitiveFieldRef() == UInternalCopies_;
        if (UnOldTimes_ >= 1)
        {
            UField_->oldTimeRef().primitiveFieldRef() == UOldInternalCopies_;
        }
        DEBUG(adapterInfo("Restored U internal-value checkpoint."));
    }
    ...
    for (uint i = 0; i < volScalarFields_.size(); i++)
    {
        ...
        *(volScalarFields_.at(i)) == *(volScalarFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            volScalarFields_.at(i)->oldTimeRef(1) == volScalarFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < volVectorFields_.size(); i++)
    {
        ...
        *(volVectorFields_.at(i)) == *(volVectorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            volVectorFields_.at(i)->oldTimeRef(1) == volVectorFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < surfaceScalarFields_.size(); i++)
    {
        *(surfaceScalarFields_.at(i)) == *(surfaceScalarFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            surfaceScalarFields_.at(i)->oldTimeRef(1) == surfaceScalarFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < surfaceVectorFields_.size(); i++)
    {
        *(surfaceVectorFields_.at(i)) == *(surfaceVectorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            surfaceVectorFields_.at(i)->oldTimeRef(1) == surfaceVectorFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < pointScalarFields_.size(); i++)
    {
        *(pointScalarFields_.at(i)) == *(pointScalarFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            pointScalarFields_.at(i)->oldTimeRef(1) == pointScalarFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < pointVectorFields_.size(); i++)
    {
        ...
        *(pointVectorFields_.at(i)) == *(pointVectorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            pointVectorFields_.at(i)->oldTimeRef(1) == pointVectorFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < volTensorFields_.size(); i++)
    {
        *(volTensorFields_.at(i)) == *(volTensorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            volTensorFields_.at(i)->oldTimeRef(1) == volTensorFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < surfaceTensorFields_.size(); i++)
    {
        *(surfaceTensorFields_.at(i)) == *(surfaceTensorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            surfaceTensorFields_.at(i)->oldTimeRef(1) == surfaceTensorFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < pointTensorFields_.size(); i++)
    {
        *(pointTensorFields_.at(i)) == *(pointTensorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            pointTensorFields_.at(i)->oldTimeRef(1) == pointTensorFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < volSymmTensorFields_.size(); i++)
    {
        *(volSymmTensorFields_.at(i)) == *(volSymmTensorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            volSymmTensorFields_.at(i)->oldTimeRef(1) == volSymmTensorFieldCopies_.at(i)->oldTime(1);
        }
    }
...
void preciceAdapter::Adapter::writeCheckpoint()
{
    ...
    // Store the velocity field U from its internal-value checkpoint
    if (UField_ != nullptr)
    {
        UInternalCopies_ = UField_->internalField();
        if (UnOldTimes_ >= 1)
...
void preciceAdapter::Adapter::readMeshCheckpoint()
{
    ...
    for (uint i = 0; i < meshSurfaceScalarFields_.size(); i++)
    {
        ...
        *(meshSurfaceScalarFields_.at(i)) == *(meshSurfaceScalarFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            meshSurfaceScalarFields_.at(i)->oldTimeRef(1) == meshSurfaceScalarFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < meshSurfaceVectorFields_.size(); i++)
    {
        ...
        *(meshSurfaceVectorFields_.at(i)) == *(meshSurfaceVectorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            meshSurfaceVectorFields_.at(i)->oldTimeRef(1) == meshSurfaceVectorFieldCopies_.at(i)->oldTime(1);
        }
    }
    ...
    for (uint i = 0; i < meshVolVectorFields_.size(); i++)
    {
        ...
        *(meshVolVectorFields_.at(i)) == *(meshVolVectorFieldCopies_.at(i));
        ...
        if (nOldTimes == 2)
        {
            meshVolVectorFields_.at(i)->oldTimeRef(1) == meshVolVectorFieldCopies_.at(i)->oldTime(1);
        }
    }
