...
preciceAdapter::FF::ThreeDGh::ThreeDGh
(
...
void preciceAdapter::FF::ThreeDGh::write(double * buffer, bool meshConnectivity, const unsigned int dim)
{
    int bufferIndex = 0;
    ...
    for (uint j = 0; j < patchIDs_.size(); j++)
    {
        ...
        forAll(p_rgh_->boundaryFieldRef()[patchID], i)
        {
            ...
            buffer[bufferIndex++]
            =
            p_rgh_->boundaryFieldRef()[patchID][i] / rho_->boundaryFieldRef()[patchID][i];
        }
    }
}

void preciceAdapter::FF::ThreeDGh::read(double * buffer, const unsigned int dim)
{
    int bufferIndex = 0;
