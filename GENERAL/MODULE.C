...
{
    DEBUG(adapterInfo("Configuring the MODULE module..."));
    ...
    {
        DEBUG(adapterInfo("Determining the solver type for the GENERAL module... (override by setting solverType to one of {compressible, incompressible, basic})"));
        solverType_ = determineSolverType();
    }
...
bool preciceAdapter::MODULE::ConjugateHeatTransfer::readConfig(const IOdictionary& adapterConfig)
{
    ...
    solverType_ = ModuleDict.lookupOrDefault<word>("solverType", "");
    ...
    nameKappa_ = ModuleDict.lookupOrDefault<word>("nameKappa", "k");
    ...
    nameRho_ = ModuleDict.lookupOrDefault<word>("nameRho", "rho");
    ...
    nameCp_ = ModuleDict.lookupOrDefault<word>("nameCp", "Cp");
    ...
    namePr_ = ModuleDict.lookupOrDefault<word>("namePr", "Pr");
    ...
    nameAlphat_ = ModuleDict.lookupOrDefault<word>("nameAlphat", "alphat");
    DEBUG(adapterInfo("    Turbulent thermal diffusivity field name for incompressible solvers : " + nameAlphat_));

    return true;
}

std::string preciceAdapter::MODULE::ConjugateHeatTransfer::determineSolverType()
{
    ...
    if (solverType == "unknown")
    {
        adapterInfo("Failed to determine the solver type. "
                    "Please specify your solver type in the MODULE section of the "
                    "preciceDict. Known solver types for GENERAL are: "
