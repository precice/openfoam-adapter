...
//----- preciceAdapter::MODULE::KappaEff_Compressible ------------------

preciceAdapter::MODULE::KappaEff_Compressible::KappaEff_Compressible(
...
void preciceAdapter::MODULE::KappaEff_Compressible::extract(uint patchID, bool meshConnectivity)
{
    if (meshConnectivity)
    {
...
scalar preciceAdapter::MODULE::KappaEff_Compressible::getAt(int i)
{
    return kappaEff_[i];
}


//----- preciceAdapter::MODULE::KappaEff_Incompressible ------------------

preciceAdapter::MODULE::KappaEff_Incompressible::KappaEff_Incompressible(
    const Foam::fvMesh& mesh,
  ...
  nameAlphat_(nameAlphat)
{
    ...
    // Get the preciceDict/GENERAL dictionary
    const dictionary& ModuleDict =
        mesh_.lookupObject<IOdictionary>("preciceDict").subOrEmptyDict("GENERAL");

    // Read the Prandtl number
    if (!ModuleDict.readIfPresent<dimensionedScalar>(namePr_, Pr_))
    {
        adapterInfo(
            "Cannot find the Prandtl number in preciceDict/GENERAL using the name " + namePr_,
            "error");
    }
    else
    {
    ...
    // Read the density
    if (!ModuleDict.readIfPresent<dimensionedScalar>(nameRho_, rho_))
    {
        adapterInfo(
            "Cannot find the density in preciceDict/GENERAL using the name " + nameRho_,
            "error");
    }
    else
    {
    ...
    if (!ModuleDict.readIfPresent<dimensionedScalar>(nameCp_, Cp_))
    {
        adapterInfo(
            "Cannot find the heat capacity in preciceDict/GENERAL using the name " + nameCp_,
            "error");
    }
    else
    {
...
void preciceAdapter::MODULE::KappaEff_Incompressible::extract(uint patchID, bool meshConnectivity)
{
...
scalar preciceAdapter::MODULE::KappaEff_Incompressible::getAt(int i)
{
    return kappaEff_[i];
}

//----- preciceAdapter::MODULE::KappaEff_Basic ---------------------------

preciceAdapter::MODULE::KappaEff_Basic::KappaEff_Basic(
    const Foam::fvMesh& mesh,
  ...
  nameKappa_(nameKappa)
{
    ...
    if (!ModuleDict.readIfPresent<dimensionedScalar>(nameKappa_, kappaEff_))
    {
        adapterInfo(
            "Cannot find the conductivity in preciceDict/GENERAL using the name " + nameKappa_,
            "error");
    }
    else
    {
...
void preciceAdapter::MODULE::KappaEff_Basic::extract(uint patchID, bool meshConnectivity)
{
    // Already extracted in the constructor
}

scalar preciceAdapter::MODULE::KappaEff_Basic::getAt(int i)
{
