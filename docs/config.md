  ...
  Interface1
  {
    ...
    (
      Velocity1
      {
        ...
        operation   value;      // Optional: Operation to perform on data (defaults to 'value', options are `gradient` and `surface-normal-gradient`)
...
The operation `gradient` is currently only supported for scalar fields. When taking the gradient of a scalar field, the resulting data will be a vector field. The operation `surface-normal-gradient` is currently only supported for surface scalar field writing. The operation `flip-normal` is supported for all field types across all modules, but the most relevant use case might be for surface scalar fields, in order to change the direction of the, e.g., flux.
